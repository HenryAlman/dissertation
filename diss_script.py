...
def simulate(
    ...
    record_video_idx: str | None = None,
    ...
    if (xml_file == "/home/henry/dissertation_5thAug/new_racecar.xml"):
        ...
        if (controller_params is None):
            ...
            controller = LeggedController(
                ...
                leg_geom_names=controller_params["leg_geom_names"]
            )
    ...
    while not done:
        ...
        if (len(info["broken_geoms"]) > 0):
            controller.inform_broken(info["broken_geoms"])
    ...
    # characteristic: number of broken legs
    broken_legs = (sum(1 for sign in controller.broken_sign_mask if sign == 0)) / 2

    # other characteristics, are ready to go from loop
...
def run_search(
    ...
    while total_elapsed_time < time_to_run:
        ...
        metrics["Total Broken Legs"]["x"].append(total_elapsed_time)
        metrics["Total Broken Legs"]["y"].append(total_broken_legs)
